...
class SE_LN(nn.Module):
    """Squeeze-and-Excitation with Layer Normalization"""
...
def oscillating_lr(optimizer, epoch, base_lr, max_lr, period):
    """Oscillating Learning Rate Scheduler"""
...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        super().__init__()
        # Maintain depthwise convolution but adjust to ensure output size matches
        self.dw = nn.Conv2d(cin, cin, 3, 2, 1, groups=cin, bias=False)  # Changed kernel size to 3 for better context
        self.maxpool = nn.MaxPool2d(2, ceil_mode=True)
        self.minpool = MinPool2d_y(2, ceil_mode=True)
        # Adjusted convolution to handle concatenated features and added padding for size consistency
        self.pw = nn.Conv2d(3*cin, cout, 1, 1, bias=False)
        self.bn = nn.BatchNorm2d(cout)
        # Added activation for non-linearity
        self.act = nn.SELU()
    ...
    def forward(self, x):
        ...
        x = self.pw(x)
        x = self.bn(x)
        x = self.act(x)  # Introduced activation
        return x
# --OPTION--
import torch
import torch.nn as nn
