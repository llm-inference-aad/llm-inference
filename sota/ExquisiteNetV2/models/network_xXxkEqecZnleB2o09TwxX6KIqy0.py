...
class Combined_SE(nn.Module):
    def __init__(self, cin, ratio=16):
        super().__init__()
        self.gavg = nn.AdaptiveAvgPool2d((1,1))
        self.gmax = nn.AdaptiveMaxPool2d((1,1))
        self.fc1 = nn.Linear(2*cin, int(2*cin / ratio))
        self.ln = nn.LayerNorm(int(2*cin / ratio))
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Linear(int(2*cin / ratio), cin)
...
class EVE(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.pw = nn.Conv2d(2*cin, cout, 1, 1, groups=2, bias=False)
        self.bn = nn.BatchNorm2d(cout)
