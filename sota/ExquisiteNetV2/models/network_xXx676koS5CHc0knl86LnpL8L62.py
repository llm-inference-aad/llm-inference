...
class DW(nn.Module):
    def __init__(self, cin, dw_s):
        ...
        # Simple Squeeze-and-Excitation Block Addition
        self.se = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(cin, cin // 4, 1, bias=True),
            nn.ReLU(),
            nn.Conv2d(cin // 4, cin, 1, bias=True),
            nn.Sigmoid()
        )
...
class CIFARNet(nn.Module):
    def __init__(self):
        ...
        # Depthwise Layers with Progressive Channel Increase
        self.dw1 = DW(16, 3)
        self.dw2 = DW(16, 3)
        self.conv_dw_transition = nn.Conv2d(16, 32, 1, bias=False)
        self.bn_dw_transition = nn.BatchNorm2d(32)
        self.act_dw_transition = nn.Hardswish()
        
        self.dw3 = DW(32, 3)
        self.dw4 = DW(32, 3)
        self.conv_dw_transition_2 = nn.Conv2d(32, 64, 1, bias=False)
        self.bn_dw_transition_2 = nn.BatchNorm2d(64)
        self.act_dw_transition_2 = nn.Hardswish()
        
        self.dw5 = DW(64, 3)
        self.dw6 = DW(64, 3)
        
        # Classification Head
        self.avgpool = nn.AvgPool2d(8)  # Assuming 32x32 input, pool to 1x1
    ...
    def forward(self, x):
        ...
        x = self.dw1(x)
        x = self.dw2(x)
        x = self.conv_dw_transition(x)
        x = self.bn_dw_transition(x)
        x = self.act_dw_transition(x)
        
        x = self.dw3(x)
        x = self.dw4(x)
        x = self.conv_dw_transition_2(x)
        x = self.bn_dw_transition_2(x)
        x = self.act_dw_transition_2(x)
