...
def get_optimizer(model, lr, weight_decay=0, nesterov=True):
    if weight_decay!= 0:
        ...
        opt = SGD(g0, lr, 0.99, nesterov=nesterov)
        opt.add_param_group({'params': g1, 'weight_decay': weight_decay, 'lr': lr * 0.9})
        opt.add_param_group({'params': g2, 'weight_decay': 0.0001, 'lr': lr * 1.1})
    else:
        opt = SGD(model.parameters(), lr, 0.99, nesterov=nesterov)
    return opt
# --OPTION--
class SE(nn.Module):
