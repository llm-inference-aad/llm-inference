...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.bn2(x)
        x = torch.max_pool2d(x, 2, 2)  # Pooling for dimension reduction
...
def get_optimizer(model, lr, weight_decay=0):
    if weight_decay != 0:
        g0, g1, g2 = [], [], []
        ...
        opt = optim.AdamW([{'params': g0}, {'params': g1, 'weight_decay': weight_decay}], lr, betas=(0.9, 0.999))
        opt.add_param_group({'params': g2})
    else:
        opt = optim.AdamW(model.parameters(), lr, betas=(0.9, 0.999))
    return opt
...
def main():
    ...
    optimizer = get_optimizer(model, lr=0.01, weight_decay=0.05)
    ...
    #     train(model, device, loader, optimizer, epoch, scheduler)
