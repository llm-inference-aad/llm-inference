...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        super().__init__()
        self.dw = nn.Conv2d(cin, cin, kernel_size=4, stride=2, padding=1, groups=cin, bias=False)
        self.maxpool = nn.MaxPool2d(kernel_size=2, ceil_mode=True)
        self.minpool = MinPool2d_y(ks=2, ceil_mode=True)
        self.pw = nn.Conv2d(3*cin, cout, kernel_size=1, bias=False)
