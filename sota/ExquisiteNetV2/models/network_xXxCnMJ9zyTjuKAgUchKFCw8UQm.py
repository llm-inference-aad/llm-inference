...
class ME(nn.Module):
    def __init__(self, cin, cout):
        super().__init__()
        # Path A: MaxPool -> PWConv -> BN
        self.path_a = nn.Sequential(
            nn.MaxPool2d(2, ceil_mode=True),
            nn.Conv2d(cin, cout, 1, 1, bias=False),
            nn.BatchNorm2d(cout)
        )
        # Path B: PWConv -> MaxPool -> BN
        self.path_b = nn.Sequential(
            nn.Conv2d(cin, cout, 1, 1, bias=False),
            nn.MaxPool2d(2, ceil_mode=True),
            nn.BatchNorm2d(cout)
        )
        # Final layers to combine outputs
        self.final_pw = nn.Conv2d(2 * cout, cout, 1, 1, bias=False)
        self.final_bn = nn.BatchNorm2d(cout)

    def forward(self, x):
        a = self.path_a(x)
        b = self.path_b(x)
        combined = torch.cat((a, b), dim=1)
        combined = self.final_pw(combined)
...
class ExquisiteNetV2(nn.Module):
    def __init__(self, class_num, img_channels):
        ...
        self.res1 = nn.Sequential(nn.Conv2d(12, 12, kernel_size=1), nn.BatchNorm2d(12))
        self.EVE = EVE(12, 48)
        self.DFSEB2 = DFSEBV2(48, 3, True)
        self.res2 = nn.Sequential(nn.Conv2d(48, 48, kernel_size=1), nn.BatchNorm2d(48))
        self.ME3 = ME(48, 96)
        self.DFSEB3 = DFSEBV2(96, 3, True)
        self.res3 = nn.Sequential(nn.Conv2d(96, 96, kernel_size=1), nn.BatchNorm2d(96))
        self.ME4 = ME(96, 192)
        self.DFSEB4 = DFSEBV2(192, 3, True)
        self.res4 = nn.Sequential(nn.Conv2d(192, 192, kernel_size=1), nn.BatchNorm2d(192))
        self.ME5 = ME(192, 384)
        self.DFSEB5 = DFSEBV2(384, 3, True)
        self.res5 = nn.Sequential(nn.Conv2d(384, 384, kernel_size=1), nn.BatchNorm2d(384))
        self.DW = DW(384, 3)
        self.gavg = nn.AdaptiveAvgPool2d((1,1))
    ...
    def forward(self, x):
        ...
        identity = x
        x = self.DFSEB1(x)
        x += self.res1(identity)
        x = self.EVE(x)
        identity = x
        x = self.DFSEB2(x)
        x += self.res2(identity)
        x = self.ME3(x)
        identity = x
        x = self.DFSEB3(x)
        x += self.res3(identity)
        x = self.ME4(x)
        identity = x
        x = self.DFSEB4(x)
        x += self.res4(identity)
        x = self.ME5(x)
        identity = x
        x = self.DFSEB5(x)
        x += self.res5(identity)
        x = self.DW(x)
        x = self.gavg(x)
        x = self.drop(x)
        x = x.view(-1, x.size(1))
