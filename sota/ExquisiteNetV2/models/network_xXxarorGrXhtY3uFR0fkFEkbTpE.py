...
class CIFARNetX(nn.Module):
    def __init__(self):
        super(CIFARNetX, self).__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, padding=1)
        self.layer_norm1 = nn.LayerNorm([64, 32, 32])
        self.resnest1 = ResNeStBlock(64, 64, stride=1)
        self.resnest2 = ResNeStBlock(64, 128, stride=2)
        self.resnest3 = ResNeStBlock(128, 256, stride=2)
        self.avgpool = nn.AvgPool2d(8)
        self.fc = nn.Linear(256, 10)

    def forward(self, x):
        x = torch.relu(self.layer_norm1(self.conv1(x)))
        x = self.resnest1(x)
        x = self.resnest2(x)
        x = self.resnest3(x)
        x = self.avgpool(x)
...
class ResNeStBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride):
        ...
        self.layer_norm1 = nn.LayerNorm([out_channels, -1, -1])  
        self.layer_norm2 = nn.LayerNorm([out_channels, -1, -1])
    ...
    def forward(self, x):
        residual = x
        out = torch.swish(self.layer_norm1(self.conv1(x)))
        out = self.conv2(out)
...
def get_optimizer(model, lr, weight_decay=0, nesterov=True, lr_multipliers=None):
    if weight_decay != 0:
        g0, g1, g2 = [], [], []  
        ...
        params = [
            {'params': g0, 'lr': lr, 'weight_decay': 0},
            ...
            for param_dict in params:
                for p in param_dict['params']:
                    if p.name in lr_multipliers:
                        param_dict['lr'] *= lr_multipliers[p.name]
        
        opt = nn.SGD(params, lr, momentum=0.9, nesterov=nesterov)
    else:
        opt = nn.SGD(model.parameters(), lr, momentum=0.9, nesterov=nesterov)
    return opt
# --OPTION--
import torch
import torch.nn as nn
