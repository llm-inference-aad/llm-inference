...
class Lookahead(Optimizer):
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                param_state = self.state[p]
                param_state['slow_params'].mul_(1 - self.alpha).add_(self.alpha * p.data)
        return loss
...
def get_optimizer(model, lr, weight_decay=0, nesterov=True, lookahead=False, max_epochs=100):
    if weight_decay!= 0:
        g0, g1, g2 = [], [], []
        ...
        opt = optim.SGD([{'params': g0, 'lr': lr,'momentum': 0.9, 'nesterov': nesterov},
                         {'params': g1, 'lr': lr*0.1, 'weight_decay': weight_decay,'momentum': 0.9, 'nesterov': nesterov},
                         {'params': g2, 'lr': lr*0.2,'momentum': 0.9, 'nesterov': nesterov}], 
                        lr=lr, momentum=0.9, nesterov=nesterov)
    else:
        opt = optim.SGD(model.parameters(), lr=lr, momentum=0.9, nesterov=nesterov)
    
    if lookahead:
        opt = Lookahead(opt, alpha=0.5, k=6)
    
    scheduler = CosineAnnealingWarmRestarts(opt, T_0=max_epochs//4, T_mult=2, eta_min=lr*0.01)
