...
class MinPool2d_y(nn.Module):
    def __init__(self, ks=2, ceil_mode=True):
        super().__init__()
        self.ks = ks
...
class FCT(nn.Module):
    def __init__(self, cin, cout, dw_ks=4, dw_stride=2, dw_padding=1, pw_ks=1, bn_momentum=0.1):
        super().__init__()
        self.dw = nn.Conv2d(cin, cin, dw_ks, dw_stride, dw_padding, groups=cin, bias=False)
        self.maxpool = nn.MaxPool2d(2, ceil_mode=True)
        self.minpool = MinPool2d_y(2, ceil_mode=True)
        self.pw = nn.Conv2d(3*cin, cout, pw_ks, 1, bias=False)
