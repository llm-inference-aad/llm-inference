...
class ResNetBlock(nn.Module):
    def __init__(self, cin, cout, stride=1, dilation=1):
        super().__init__()
        self.conv1 = nn.Conv2d(cin, cout, kernel_size=3, padding=dilation, stride=stride, dilation=dilation)
        self.bn1 = nn.BatchNorm2d(cout)
        self.act1 = nn.SiLU()  
        self.conv2 = nn.Conv2d(cout, cout, kernel_size=3, padding=dilation, dilation=dilation)
...
class EnhancedNet(nn.Module):
    def __init__(self):
        ...
        self.block1 = ResNetBlock(64, 64, stride=1, dilation=1)
        
        self.block2 = ResNetBlock(64, 128, stride=1, dilation=2)  
  
        self.block3 = ResNetBlock(128, 256, stride=1, dilation=4)  
  
        self.gap = nn.AdaptiveAvgPool2d((1,1))
        self.fc = nn.Linear(256, 10)
    
    def forward(self, x):
        x = self.act_stem(self.stem(x))
...
class Net(nn.Module):
    def __init__(self):
        ...
        # Block 1
        self.conv1 = nn.Conv2d(64, 64, kernel_size=3, padding=1, groups=64, bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.se1 = SE_LN(64)
        self.act2 = nn.SiLU()
        
        # Block 2 (Dilated)
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, padding=2, dilation=2, groups=64, bias=False)
        self.bn2 = nn.BatchNorm2d(128)
        self.se2 = SE_LN(128)
        self.act3 = nn.SiLU()
        
        # Block 3 (Deeper, Dilated)
        self.conv3 = nn.Conv2d(128, 256, kernel_size=3, padding=4, dilation=4, groups=128, bias=False)
    ...
    def forward(self, x):
        x = self.act1(self.stem(x))
        
        x = self.act2(self.bn1(self.conv1(x)))
        x = self.se1(x)
        
        x = self.act3(self.bn2(self.conv2(x)))
        x = self.se2(x)
        
        x = self.act4(self.bn3(self.conv3(x)))
        x = self.se3(x)
...
class Network(nn.Module):
    def __init__(self):
        super().__init__()
        self.block1 = FCT(3, 64)
        self.block2 = FCT(64, 128)
        self.block3 = FCT(128, 256)
