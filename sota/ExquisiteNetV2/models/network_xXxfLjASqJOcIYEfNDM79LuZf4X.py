...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        super().__init__()
        # Introduce Depthwise Separable Convolution for dw
        self.dw = nn.Sequential(
            nn.Conv2d(cin, cin, 4, 2, 1, groups=cin, bias=False),  # Depthwise
            nn.Conv2d(cin, cin, 1, 1, bias=False)                   # Pointwise (Adjusting to keep output size)
        )
        self.maxpool = nn.MaxPool2d(2, ceil_mode=True)
        self.minpool = MinPool2d_y(2, ceil_mode=True)
        # Adjust pw to accommodate the new channel count after concatenation
    ...
    def forward(self, x):
        x = torch.cat((
            self.maxpool(x),
            self.minpool(x),
            self.dw(x),  # Utilizing the new DWSC
        ), 1)
        x = self.pw(x)
        x = self.bn(x)
