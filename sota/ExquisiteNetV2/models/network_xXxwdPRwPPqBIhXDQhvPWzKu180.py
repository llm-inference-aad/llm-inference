...
class SE(nn.Module):
    def __init__(self, cin, ratio):
        ...
        self.act1 = nn.ReLU(inplace=True) # Using inplace for efficiency
    ...
    def forward(self, x):
        y = x
        x = self.gavg(x)
        x = x.view(-1,x.size()[1]) # Flattening for linear layers
        x = self.fc1(x)
        x = self.act1(x)
        x = self.fc2(x)
        x = self.act2(x)
        x = x.view(-1,x.size()[1],1,1) # Reshaping back to original dimensions
        return x * y # Element-wise multiplication instead of *
# --OPTION--
