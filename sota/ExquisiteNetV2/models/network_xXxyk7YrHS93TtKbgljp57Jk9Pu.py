...
class ExquisiteNetV2(nn.Module):
    def __init__(self, class_num, img_channels):
        super().__init__()
        self.FCT = FCT(img_channels, 16)  # Increased output channels
        self.DFSEB1 = DFSEBV2(16, 3, True)
        self.EVE = EVE(16, 64)            # Adjusted input/output
        self.DFSEB2 = DFSEBV2(64, 3, True)
        self.ME3 = ME(64, 128)           # Doubled output channels
        self.DFSEB3 = DFSEBV2(128, 3, True)
        self.ME4 = ME(128, 256)          # Doubled output channels
        self.DFSEB4 = DFSEBV2(256, 3, True)
        self.ME5 = ME(256, 512)          # Doubled output channels
        self.DFSEB5 = DFSEBV2(512, 3, True)
        self.DW = DW(512, 3)
        self.gavg = nn.AdaptiveAvgPool2d((1,1))
        self.drop = nn.Dropout(0.7)       # Increased dropout rate
