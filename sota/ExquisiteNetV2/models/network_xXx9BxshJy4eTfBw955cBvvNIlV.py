...
class ChannelAttention(nn.Module):
    def __init__(self, num_channels):
        ...
        self.fc = nn.Sequential(
            nn.Linear(num_channels, num_channels // 4, bias=False),
            nn.ReLU(),
            nn.Linear(num_channels // 4, num_channels, bias=False),
            nn.Sigmoid()
        )
...
class EVE(nn.Module):
    def __init__(self, cin, cout, skip=False):
        ...
        self.bn = nn.BatchNorm2d(cout)
        self.act = nn.SELU() if cin < 128 else nn.GELU()
        self.skip = skip
        if self.skip:
            self.scale = nn.Parameter(torch.ones(1))

    def forward(self, x):
        x_pool = torch.cat((self.maxpool(x), self.minpool(x)), 1)
        x_conv = self.pw(x_pool)
        x_bn = self.bn(x_conv)
        x_act = self.act(x_bn)
        
        if self.skip and x.shape[1] == x_act.shape[1]:
            return x_act + self.scale * x
        else:
            return x_act
...
class CIFAR10Model(nn.Module):
    def __init__(self):
        ...
        self.eve1 = EVE(64, 64, skip=True)
        self.eve2 = EVE(64, 64)
        self.ca1 = ChannelAttention(64)
        self.eve3 = EVE(64, 128, skip=True)
        self.eve4 = EVE(128, 128)
        self.ca2 = ChannelAttention(128)
        self.eve5 = EVE(128, 256, skip=True)
        self.eve6 = EVE(256, 256)
        self.ca3 = ChannelAttention(256)
        
        self.dwsep_conv = nn.Sequential(
