...
def pad_num_y(k_s):
    """Calculates padding for depthwise convolution."""
    pad_per_side = int((k_s-1)*0.5)
    return pad_per_side
...
class DW(nn.Module):
    ...
    Args:
        ...
        dw_s (int): Size of the depthwise convolution kernel.
    """

    def __init__(self, cin, dw_s):
        super().__init__()
        # Initialize depthwise convolution layer with specified kernel size and padding.
    ...
    def forward(self, x):
        ...
        Returns:
            torch.Tensor: Output tensor after applying depthwise convolution and activation.
        """
        x = self.dw(x)  # Apply depthwise convolution.
        x = self.act(x)  # Apply Hardswish activation.
        return x
# --OPTION--
def pad_num_y(k_s):
    pad_per_side = int((k_s-1)*0.5)
