...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.ln = nn.LayerNorm([cout, 1, 1])  # Added LayerNorm
        self.relu = nn.ReLU(inplace=True)     # Added ReLU activation

    def forward(self, x):
        max_out = self.maxpool(x)
        min_out = self.minpool(x)
        dw_out = self.dw(x)
        x = torch.cat((max_out, min_out, dw_out), 1)
        x = self.pw(x)
        x = self.ln(x)  # Applied LayerNorm
        x = self.relu(x)  # Applied ReLU
        return x
# --OPTION--
