...
class SE_LN(nn.Module):
    def __init__(self, cin, ratio=2):
        ...
        self.fc1 = nn.Linear(cin, cin // ratio)
        self.ln = nn.LayerNorm(cin // ratio)
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Linear(cin // ratio, cin)
        self.act = nn.Sigmoid()
...
class MinPool2d_x(nn.Module):
    ...
    def forward(self, x):
        return -F.max_pool2d(-x, self.ks, padding=1, ceil_mode=self.ceil_mode)
...
class EVE(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.pw = nn.Conv2d(2*cin, cout, 1, 1, bias=True)
        self.bn = nn.BatchNorm2d(cout)
