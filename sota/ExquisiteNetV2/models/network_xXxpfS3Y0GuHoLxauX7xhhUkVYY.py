...
class SE_LN(nn.Module):
    def __init__(self, cin, reduction_ratio=16):
        super().__init__()
        self.gavg = nn.AdaptiveAvgPool2d((1,1))  
        self.fc_reduce = nn.Linear(cin, cin // reduction_ratio, bias=False)
        self.fc_expand = nn.Linear(cin // reduction_ratio, cin, bias=False)
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w = x.size()
        x_ = x.view(-1, 1, h, w)  # Collapse channels for spatial attention
        x_ = self.conv(x_)
        x_ = self.sigmoid(x_)
        x_ = x_.view(b, c, h, w)
        return x_ * x
...
class CIFARNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.blocks = nn.ModuleList([
            DepthSepConvBlock(32, 64, stride=2), 
            SE_LN(64), 
            SpatialAttention(),
            DepthSepConvBlock(64, 128, stride=2), 
            SE_LN(128), 
            SpatialAttention(),
            DepthSepConvBlock(128, 256, stride=2), 
            SE_LN(256), 
            SpatialAttention()
        ])
...
class DFSEBV2(nn.Module):
    def __init__(self, cin, dw_s, is_LN):
        ...
        self.dca1 = DynamicChannelAttention(cin)  # --- Added DCA Helper
        if is_LN:
            self.seln = SE_LN(cin)
        else:
            self.seln = SE(cin,3)
        ...
        self.dca2 = DynamicChannelAttention(cin)  # --- Added DCA Helper
