...
class SGD(torch.optim.SGD):
    def __init__(self, params, lr, momentum=0, dampening=0,
        ...
        super(SGD, self).__init__(params, lr, momentum=momentum, dampening=dampening,
                                  weight_decay=weight_decay, nesterov=nesterov)

    def add_param_group(self, param_group):
        assert isinstance(param_group, dict), "param group must be a dict"
        param_group['lr'] = param_group.get('lr', self.param_groups[0]['lr'])
        param_group['momentum'] = param_group.get('momentum', self.param_groups[0]['momentum'])
        param_group['dampening'] = param_group.get('dampening', self.param_groups[0]['dampening'])
        param_group['weight_decay'] = param_group.get('weight_decay', self.param_groups[0]['weight_decay'])
        param_group['nesterov'] = param_group.get('nesterov', self.param_groups[0]['nesterov'])
...
if __name__ == "__main__":
    ...
    optimizer = get_optimizer(model, lr=0.01, weight_decay=0.0001, nesterov=True)
    print(optimizer)
# --OPTION--
class SE(nn.Module):
