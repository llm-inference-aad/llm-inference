...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.depthwise_conv = nn.Conv2d(cin, cin, 4, 2, 1, groups=cin, bias=False)
        self.pointwise_conv = nn.Conv2d(3 * cin, cout, 1, 1, bias=False)
        self.batchnorm = nn.BatchNorm2d(cout)

    def forward(self, x):
        max_pooled = self.maxpool(x)
        min_pooled = self.minpool(x)
        depthwise = self.depthwise_conv(x)
        
        concatenated = torch.cat((max_pooled, min_pooled, depthwise), dim=1)
        pointwise = self.pointwise_conv(concatenated)
        output = self.batchnorm(pointwise)
...
class FCT(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.maxpool = nn.MaxPool2d(2, ceil_mode=True)
        self.minpool = MinPool2d_y(2, ceil_mode=True)
        self.pw = nn.Conv2d(3*cin, cout, 1, 1, bias=True)
        self.bn = nn.BatchNorm2d(cout, momentum=0.01)

    def forward(self, x):
        x_dw = self.dw(x)
        x_dw_relu = F.relu(x_dw)
        max_out = self.maxpool(x)
        min_out = self.minpool(x)
        concat_x = torch.cat((max_out, min_out, x_dw_relu), 1)
        concat_x_relu = F.relu(concat_x)
        x_pw = self.pw(concat_x_relu)
        x_bn = self.bn(x_pw)
        return x_bn
# --OPTION--
