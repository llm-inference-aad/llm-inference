...
class SE_LN(nn.Module):
    ...
    def forward(self, x):
        ...
        avg_x = self.gavg(x).view(-1, x.size(1))
        avg_x = self.ln(avg_x).view(-1, x.size(1), 1, 1)
        gate = self.act(avg_x)
...
class EVE(nn.Module):
    def __init__(self, cin, cout):
        ...
        # Depthwise Separable Convolution with Enhanced Activation
        ...
        self.bn = nn.BatchNorm2d(cout)
        self.act = nn.SiLU()  # Swapped to SiLU for better non-linearity
...
# --OPTION--
class ME(nn.Module):
    def __init__(self, cin, cout, use_se=False, use_activation_stack=False):
        super().__init__()
        self.maxpool = nn.MaxPool2d(2, ceil_mode=True)
        self.pw = nn.Conv2d(cin, cout, 1, 1, bias=False)
        self.bn = nn.BatchNorm2d(cout)
        self.se = SE_LN(cout, use_gate=True) if use_se else None
...
class ReversibleBlock(nn.Module):
    ...
    def forward(self, x):
        x_a, x_b = torch.split(x, x.shape[1]//2, dim=1)
        x_b = self.f(x_a) + x_b
        x_a = self.g(x_b) + x_a
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        avg_feat = x.squeeze(2).squeeze(2)
        attention = self.mlp(avg_feat)
        attention = torch.sigmoid(attention)
        x_weighted = x * attention.unsqueeze(2).unsqueeze(3)
        return x_weighted
...
class ExquisiteNetV2(nn.Module):
    def __init__(self, class_num, img_channels):
        ...
        self.ME3 = ME(48, 96)
        self.rev_block3 = ReversibleBlock(
            f=nn.Conv2d(48, 48, kernel_size=3, padding=1),
            g=nn.Conv2d(48, 48, kernel_size=3, padding=1)
        )
        self.DFSEB3 = DFSEBV2(96, 3, True)
        self.ME4 = ME(96, 192)
        self.rev_block4 = ReversibleBlock(
            f=nn.Conv2d(96, 96, kernel_size=3, padding=1),
            g=nn.Conv2d(96, 96, kernel_size=3, padding=1)
        )
        self.DFSEB4 = DFSEBV2(192, 3, True)
        self.ME5 = ME(192, 384)
        self.rev_block5 = ReversibleBlock(
            f=nn.Conv2d(192, 192, kernel_size=3, padding=1),
            g=nn.Conv2d(192, 192, kernel_size=3, padding=1)
        )
