...
class EVE(nn.Module):
    def __init__(self, cin, cout):
        ...
        self.minpool = MinPool2d_x(ks=2, ceil_mode=True)
        self.pw = nn.Conv2d(in_channels=2*cin, out_channels=cout, kernel_size=1, padding=0, bias=False)
        self.bn = nn.BatchNorm2d(num_features=cout)

    def forward(self, x):
        x = torch.cat((self.maxpool(x), self.minpool(x)), dim=1)
        x = self.pw(x)
        x = self.bn(x)
        return x
# --OPTION--
