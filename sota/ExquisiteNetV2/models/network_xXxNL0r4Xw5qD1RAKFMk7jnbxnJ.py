...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.tensor([keep_prob]).to(x.device).expand(x.size(0), -1, -1, -1)
        mask = torch.bernoulli(mask).div(keep_prob)
        return x * mask

class ResNetBlock(nn.Module):
    def __init__(self, cin, cout, stride=1, drop_path_prob=0.0):
        super().__init__()
        self.conv1 = nn.Conv2d(cin, cout, kernel_size=3, padding=1, stride=stride)
        self.bn1 = nn.BatchNorm2d(cout)
        self.act1 = Swish() # Changed to Swish
        self.conv2 = nn.Conv2d(cout, cout, kernel_size=3, padding=1)
        self.bn2 = nn.BatchNorm2d(cout)
        self.act2 = Swish() # Changed to Swish
...
class CIFAR10Model(nn.Module):
    def __init__(self, drop_path_probs=[0.0]*6): # Example probs, adjust based on depth
        super().__init__()
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, padding=1)
        self.bn1 = nn.BatchNorm2d(64)
        self.act1 = nn.ReLU()
        self.blocks = nn.ModuleList([
            ResNetBlock(64, 64, stride=1, drop_path_prob=drop_path_probs[0]),
            ResNetBlock(64, 64, stride=1, drop_path_prob=drop_path_probs[1]),
            ResNetBlock(64, 128, stride=2, drop_path_prob=drop_path_probs[2]),
            ResNetBlock(128, 128, stride=1, drop_path_prob=drop_path_probs[3]),
            ResNetBlock(128, 256, stride=2, drop_path_prob=drop_path_probs[4]),
            ResNetBlock(256, 256, stride=1, drop_path_prob=drop_path_probs[5]),
        ])
...
# Example training setup (not part of the model, but for completeness)
def train(model, device, loader, epoch):
...
class ME(nn.Module):
    def __init__(self, cin, cout):
        super().__init__()
        self.maxpool = nn.MaxPool2d(2, ceil_mode=True)
        self.dw = nn.Conv2d(cin, cin, kernel_size=3, groups=cin, padding=1, bias=False)
...
# Hyperparameter Strategies for Training
