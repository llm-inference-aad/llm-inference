...
# RAG memory store: persist and retrieve past interactions (queries, responses, outcomes) for episodic context
RAG_MEMORY_STORE_ENABLED = os.environ.get("RAG_MEMORY_STORE_ENABLED", "false").lower() in {"1", "true", "yes"}
RAG_MEMORY_TOP_K = int(os.environ.get("RAG_MEMORY_TOP_K", 3))  # Number of memory entries to retrieve per prompt
