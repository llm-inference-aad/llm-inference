...
def extract_json(content):
    try:
        ...
        json_content = json_content.replace('\\_', '_')  # Fix LaTeX-style escaped underscores
        import re as _re
        json_content = _re.sub(r'\bNone\b', 'null', json_content)  # Replace Python None with JSON null (word-boundary safe)
        ...
        log.warning("JSON parse attempt 1 failed: %s", e)
        # Try removing trailing commas (e.g. ", }" or ",}")
        try:
            json_content = _strip_trailing_commas(json_content)
            return json.loads(json_content)
        except:
            pass
        # Try fixing single-quoted JSON (common with local models)
        try:
            fixed = _fix_single_quoted_json(json_content)
            fixed = _strip_trailing_commas(fixed)
            return json.loads(fixed)
        except:
            log.error("extract_json FAILED — raw LLM output:\n%s", content)
            return {}
    except Exception as e:
        log.error("extract_json unexpected error: %s — raw LLM output:\n%s", e, content)
        return {}
