...
#SBATCH --output=/home/hice1/jgil37/scratch/llm-inference/slurm_logs/vllm_smoke_%j.out
#SBATCH --error=/home/hice1/jgil37/scratch/llm-inference/slurm_logs/vllm_smoke_%j.err
...
cd "$ROOT_DIR"
mkdir -p slurm_logs runs/vllm_smoke/metrics runs/vllm_smoke/logs runs/vllm_smoke/errors

# Use the adaptive vLLM config as the base.
cp .env.vllm_all .env

# Make the smoke workload truly mixed: request-level constraints are still used
# for the constrained cases, but the server should not force a default JSON
# constraint onto every request.
...
# Force float16 on older GPUs that don't support bfloat16 (e.g. V100)
...
# Run only the 5-request smoke workload against the live vLLM server.
SERVER_PORT=8001 python run_five_config_tests.py --port 8001 --output-dir "$ROOT_DIR/runs/$RUN_ID/metrics"

# Leave outputs on disk; cleanup trap will stop the server.
echo "[INFO] Smoke workload complete"
